...
} from "@/lib/domain/glucose";

export async function POST(request: Request) {
  const auth = await authenticatedRequest();
  if (auth.response) return auth.response;
  if (await rateLimited(auth.supabase, "glucose.create", 30))
  ...
  return error
    ? safeDatabaseFailure("Catatan belum dapat disimpan.")
    : success(data, 201);
}
...
export async function GET(request: Request) {
  const auth = await authenticatedRequest();
  if (auth.response) return auth.response;
  if (await rateLimited(auth.supabase, "glucose.history", 120))
    return failure("RATE_LIMITED", "Terlalu banyak permintaan.", 429);
  const url = new URL(request.url);
  const cursorParam = url.searchParams.get("cursor");
  const cursor = decodeCursor(cursorParam);
  if (cursorParam && !cursor)
    return failure("INVALID_CURSOR", "Cursor tidak valid.", 400);
  ...
  let query = auth.supabase
    ...
    .select(
      "id,original_value,original_unit,normalized_mg_dl,measurement_context,measured_at,recorded_at,note,status,invalidated_at,invalidation_reason,replacement_for_id",
    )
  ...
  if (cursor) {
    query = query.or(
      `measured_at.lt.${cursor.measuredAt},and(measured_at.eq.${cursor.measuredAt},id.lt.${cursor.id})`,
    );
  }
