...
export function validateGlucoseInput(value: unknown, now = new Date()) {
  ...
  const measuredAt =
    typeof input?.measuredAt === "string" ? input.measuredAt : "";
  const note =
    typeof input?.note === "string" ? input.note.trim() || null : null;
  const measuredDate = new Date(measuredAt);

  if (
    !Number.isFinite(originalValue) ||
    originalValue <= 0 ||
    Math.abs(Math.round(originalValue * 1000) - originalValue * 1000) > 1e-9
  ) {
    errors.originalValue =
      "Nilai harus positif dengan maksimal tiga angka desimal.";
  }
  if (originalUnit !== "mg/dL" && originalUnit !== "mmol/L")
    errors.originalUnit = "Satuan tidak valid.";
  if (!CONTEXTS.includes(measurementContext as never))
    errors.measurementContext = "Kondisi pengukuran tidak valid.";
  if (
    Number.isNaN(measuredDate.valueOf()) ||
    measuredDate.valueOf() > now.valueOf() + 5 * 60_000
  ) {
    errors.measuredAt =
      "Waktu pengukuran tidak valid atau terlalu jauh di masa depan.";
  }
  if (note && note.length > 1000)
    errors.note = "Catatan maksimal 1000 karakter.";
...
function zonedDate(now: Date, offsetMinutes: number) {
  return new Date(now.valueOf() + offsetMinutes * 60_000)
    .toISOString()
    .slice(0, 10);
}
...
function utcBoundary(localDate: string, offsetMinutes: number) {
  return (
    new Date(`${localDate}T00:00:00.000Z`).valueOf() - offsetMinutes * 60_000
  );
}
...
export function resolveRange(
  ...
  if (period === "current_month" || month) {
    const selectedMonth = month ?? zonedDate(now, offset).slice(0, 7);
    if (
      !/^\d{4}-(0[1-9]|1[0-2])$/.test(selectedMonth) ||
      !dateOnly(selectedMonth + "-01")
    )
      return null;
  ...
  if (!from || !to || !dateOnly(from) || !dateOnly(to) || from > to)
    return null;
  ...
  return {
    ...
    toExclusive: new Date(
      utcBoundary(afterTo.toISOString().slice(0, 10), offset),
    ).toISOString(),
  };
}
